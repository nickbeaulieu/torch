function defineAst(start: string, baseName: string, definitions: string[]) {
  ...
  output += "  abstract accept<R>(visitor: Visitor<R>): R;\n";

  output += "}\n";

  output += defineVisitor(baseName);
...
function defineType(baseName: string, className: string, fieldList: string) {
  ...
  output += `    return visitor.visit(this);\n`;
  output += "  }\n";
  output += "}\n";
  return output;
}

function defineVisitor(baseName: string) {
  let output = "";
  output += "export interface Visitor<R> {\n";
  output += `  visit(${baseName.toLowerCase()}: ${baseName}): R;\n`;
  output += "}\n";
  return output;
}
...
defineAst(AUTOGENERATED + TOKEN + EXPR, "Stmt", [
  ...
  "Func = name: Token, params: Token[], body: Stmt[], returnType: Token | null",
